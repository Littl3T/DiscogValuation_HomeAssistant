"""Orchestration d'un snapshot complet. Aucune intervention utilisateur.

Le budget de requetes est le fil conducteur : Discogs plafonne a 60/min, donc
tout ce qui peut etre mis en cache l'est, et l'etage 2 est plafonne par run.

Cout d'un rafraichissement, collection de 145 pressages :
  cache froid  ~440 requetes (~8 min)  premier lancement uniquement
  cache chaud  ~5 requetes             tous les suivants
"""
...
class ValuationCoordinator(DataUpdateCoordinator[dict[str, Any]]):
    """Enchaine collecte, valorisation, correction et ecriture."""
    ...
    def store(self) -> ValuationStore:
        """Expose la persistance aux services de reporting."""
        return self._store

    # -- etapes -----------------------------------------------------------

    async def _resolve_fields(self) -> None:
        """Les field_id des conditions sont propres a chaque compte Discogs."""
    ...
    async def _price(
        ...
        """Prix de base NM. L'API renvoie 8 etats mais ils derivent d'une base
        unique : on n'en stocke qu'une valeur."""
    ...
    async def _signals(
        ...
        """Signaux de confiance. Deux appels : /releases pour les metadonnees,
        /marketplace/stats pour le plancher dans la bonne devise.

        lowest_price de /releases est inutilisable : toujours en USD, sans
        champ currency, et curr_abbr y est ignore.
        """
    ...
    async def _band(self, master_id: int, cache: dict[int, float]) -> float | None:
        """Etage 2 : mediane des pressages freres les plus possedes.

        Cout ~10 requetes, d'ou le declenchement selectif et le cache long.
        """
        ...
        versions: list[dict] = []
        page = 1
        while page <= 2:  # les 2 premieres pages suffisent au top possede
    ...
    async def _async_update_data(self) -> dict[str, Any]:
        try:
            ...
            raise UpdateFailed(f"Token Discogs refuse: {err}") from err
        except Exception as err:  # noqa: BLE001 - remonte proprement dans HA
            raise UpdateFailed(f"Echec du snapshot: {err}") from err

    async def _snapshot(self) -> dict[str, Any]:
        await self._resolve_fields()
        items = await self._fetch_collection()
        if not items:
            raise UpdateFailed("Collection vide ou illisible")
        ...
        for release_id in candidates:
            ...
            if spent >= STAGE2_BUDGET:
                _LOGGER.debug("Budget etage 2 epuise, report au prochain run")
                break
        ...
        # Conversion de devise si le compte Discogs n'est pas dans la devise
        # demandee. Le taux est fige dans le snapshot.
        ...
        if source_currency != self._currency:
            ...
            if rate is None:
                _LOGGER.warning(
                    "Conversion %s -> %s impossible, valeurs conservees en %s",
        ...
        _LOGGER.info(
            "Snapshot: %s items, %s %s, %s requetes",
