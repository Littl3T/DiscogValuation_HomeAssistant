"""SQLite persistence: the table, the view and the caches.

Deliberately outside the Home Assistant database. The recorder purges after
10 days by default and its schema is private: writing there would break at the
first minor update. We own our own file, in the configuration directory, so it
is included in HA backups.

Every method is synchronous and blocking: the caller MUST run them through
...
-- Keyed on instance_id: the same pressing can be owned as several copies,
...
class ValuationStore:
    """SQLite access. One connection per call: simple and safe in an executor."""
    ...
    def write_snapshot(
        ...
        """Write a complete snapshot in one transaction."""
    ...
    def like_for_like_delta(self) -> dict[str, Any] | None:
        """Like-for-like change, over the intersection of copies.

        Without it, buying a EUR 40 record shows +40 and reads as market
        appreciation. This metric separates the two.
        """
        with self._connect() as conn:
            ids = [
                r["id"]
    ...
    def report_history(self, limit: int = 365) -> list[dict[str, Any]]:
        """The valuation_history view, most recent first."""
        with self._connect() as conn:
            return [
                dict(r)
                for r in conn.execute(
    ...
    def report_items(
        ...
        """Per-copy detail for one snapshot.

        order_by is checked against an allowlist: it comes from a service call,
        therefore from user input.
        """
    ...
    def report_flagged(self, snapshot_id: int | None = None) -> list[dict[str, Any]]:
        """Copies that were corrected or are unreliable: the ones to look at."""
        with self._connect() as conn:
            if snapshot_id is None:
                row = conn.execute(
                    "SELECT MAX(id) FROM snapshot WHERE status='complete'"
    ...
    def report_movers(self, limit: int = 20) -> list[dict[str, Any]]:
        """Largest movers between the last two snapshots.

        Like-for-like: only copies present in both.
        """
        with self._connect() as conn:
            ids = [
                r["id"]
    ...
    def purge(self, keep_detail_days: int = 90) -> int:
        """Drop per-item detail from old snapshots, keep the aggregates.

        ~1 row per record per snapshot: without this, 1,500 records on a daily
        schedule make 550k rows a year.
        """
