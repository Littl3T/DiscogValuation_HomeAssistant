"""Persistence tests: schema, view, like-for-like change.

The store has no dependency on Home Assistant, so it is tested directly.
"""
...
class TestSchema:
    ...
    def test_idempotent_init(self, tmp_path: Path):
        ...
        ValuationStore(path)
        ValuationStore(path)  # must not raise
...
class TestView:
    def test_aggregates_single_snapshot(self, store: ValuationStore):
        ...
        assert row["delta_abs"] is None  # no previous snapshot

    def test_delta_between_snapshots(self, store: ValuationStore):
        import time

        _write(store, [_row(1, 100.0), _row(2, 50.0)])
        time.sleep(1.1)  # the view orders on ts, second precision
...
class TestLikeForLike:
    ...
    def test_separates_market_move_from_purchase(self, store: ValuationStore):
        """Buying a record must not read as a market rise."""
        import time

        _write(store, [_row(1, 100.0)])
        time.sleep(1.1)
        # The existing record gains 10%, and we buy one at 500.
        _write(store, [_row(1, 110.0), _row(2, 500.0)])

        history = store.history()[0]
        assert history["delta_abs"] == 510.0  # raw: mixes both effects
...
class TestCaches:
    ...
    def test_null_price_not_returned(self, store: ValuationStore):
        """A missing price is remembered but never surfaces as a value."""
...
class TestReporting:
    """The methods that feed the reporting services."""
    ...
    def test_items_returns_detail(self, store: ValuationStore):
        ...
        assert len(items) == 2
        assert items[0]["value"] == 100.0  # descending sort by default
    ...
    def test_items_rejects_injection_in_order_by(self, store: ValuationStore):
        """order_by comes from a service call: it must be allowlisted."""
        _write(store, [_row(1, 100.0)])
    ...
    def test_movers_ranks_by_absolute_change(self, store: ValuationStore):
        ...
        assert len(movers) == 2  # the third one did not move
    ...
    def test_movers_ignores_new_items(self, store: ValuationStore):
        """A record bought between two snapshots is not a variation."""
        import time
